...
- Real texture icons when the dataset pipeline can match Minecraft PNG assets from the
  selected GTNH mods. Missing icons remain blank; the app does not invent item art.
...
- Static texture lookup cannot fully render every GregTech metaitem or NBT-dependent
  stack. Those need a future client-side item-stack render exporter.
...
It then scans the same GTNH instance/mod jars for real PNG textures under
`assets/<modid>/textures/items`, `blocks`, and `fluids`, copies matched assets to
`$GTNH_DATASET_OUT_DIR/textures`, and writes `iconPath` references into the normalized
dataset. No placeholder or generated icons are published.
